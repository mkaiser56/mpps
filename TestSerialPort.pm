...
#Test    my $written = $self->write_bg ($wbuf);
    my $written = length($wbuf);
    my $TestCommand = $wbuf;
#Test    if ($written != $lbuf) {
#Test        ($ok, $written) = $self->write_done(1);	# block until done
#Test    }
    if ($Verbose) {
	print "wbuf=$wbuf\n";
	print "lbuf=$lbuf\n";
...
TestSerialPort - User interface to Win32 Serial API calls

=head1 SYNOPSIS

  require 5.003;
  use TestSerialPort qw( :STAT 0.19 );

=head2 Constructors

  $PortObj = new TestSerialPort ($PortName, $quiet)
       || die "Can't open $PortName: $^E\n";    # $quiet is optional

  $PortObj = start TestSerialPort ($Configuration_File_Name)
       || die "Can't start $Configuration_File_Name: $^E\n";

  $PortObj = tie (*FH, 'TestSerialPort', $Configuration_File_Name)
...
=head2 Configuration Utility Methods
  ...
  @necessary_param = TestSerialPort->set_test_mode_active(1);
...
the same I<Physical> hardware:
  ...
  $P1 = new TestSerialPort ("COM2");

  $P2 = new TestSerialPort ("\\\\.\\Nanohertz Modem model K-9");
...
of a validity check. The returned object is ready for access.
  ...
  $PortObj2 = start TestSerialPort ($Configuration_File_Name)
       || die;

The third constructor, B<tie>, combines the B<start> with Perl's
support for tied FileHandles (see I<perltie>). TestSerialPort
...
support was new with Version 0.14.
  ...
  $PortObj2 = tie (*FH, 'TestSerialPort', $Configuration_File_Name)
       || die;
...
The tied FileHandle methods may be combined with the TestSerialPort
...
time in TestSerialPort, you should always use B<sysread> with the
...
You need something like this (Perl 5.005):
        ...
        # construct
    $tie_ob = tie(*FOO,'TestSerialPort', $cfgfile)
